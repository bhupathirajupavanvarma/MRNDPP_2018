...
def test_raw_strings_are_strings():
    ...
    string1="Hello\ n"
    assert  string1.replace(" ","")== r"Hello\n"
...
def test_string_membership():
    assert 'c' not in 'apple'  #is there a precedence issue here or something else?
    assert 'a' in 'apple'
    assert 'app' in 'apple'
...
def rotate_right(input, count):
    ...
    return input[len(input)-count:] +input[:len(input)-count]
