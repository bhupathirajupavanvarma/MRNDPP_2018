...
def get_encoding_dict():
    ...
    """
    a=dict(zip(string.ascii_lowercase,list(range(1,27))))
    b=dict(zip(string.ascii_uppercase,list(range(1,27))))
    a.update(b)
    return a
...
def capitalize_names(input):
    ...
    """
    return list(input[i].title() for i in range(len(input)))
    #s=",".join(input)
    #s=s.title();
    #s=list(s.split(","))
    #return s; #not able to handle empty string
